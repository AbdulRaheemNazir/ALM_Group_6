...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($recaptcha_result['success']) { // Check only the success field for v2
        ...
        $stmt = $db->prepare("SELECT * FROM library_member WHERE email = :email");
        ...
        if ($user) {
            ...
            if ($user['email_verified'] == 1) { // Assuming 'email_verified' is the column name
                // Verify the password
                if (password_verify($password, $user['password_hash'])) {
                    // Password is correct, set session variables
                    $_SESSION['member_id'] = $user['member_id'];
                    $_SESSION['first_name'] = $user['first_name'];
                    $_SESSION['role'] = 'Member';  // Optional: You can store the user role

                    // Redirect to the member's dashboard or homepage
