...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($recaptcha_result['success']) { // Check only the success field for v2
        ...
        // Prepare a SQL query to find the user by email in the 'purchase_manager' table
        $stmt = $db->prepare("SELECT * FROM purchase_manager WHERE email = :email");
        ...
        if ($user) {
            ...
            if (password_verify($password, $user['password_hash'])) {
                ...
                $_SESSION['purchase_manager_id'] = $user['purchase_manager_id'];
                $_SESSION['first_name'] = $user['first_name'];
                $_SESSION['email'] = $user['email'];

                // Redirect to the purchase manager's dashboard or homepage
                header("Location: purchasemanager_dashboard.php");
                exit();
            } else {
...
<body>
    <div class="container">
        ...
        <div class="about-section">
            ...
            <form action="login_purchasemanager.php" method="post">
