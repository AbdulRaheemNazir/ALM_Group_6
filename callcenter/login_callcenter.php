...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($recaptcha_result['success']) { // Check only the success field for v2
        ...
        // Prepare a SQL query to find the user by email in the 'call_centre_operator' table
        $stmt = $db->prepare("SELECT * FROM call_centre_operator WHERE email = :email");
        ...
        if ($user) {
            // Check if the email has been verified
                ...
                if (password_verify($password, $user['password_hash'])) {
                    ...
                    $_SESSION['call_centre_operator_id'] = $user['call_centre_operator_id'];
                    $_SESSION['first_name'] = $user['first_name'];
                    $_SESSION['email'] = $user['email'];

                    // Redirect to the call centre operator's dashboard or homepage
                    header("Location: call_centre_operator_dashboard.php");
                    exit();
                } else {
...
<body>
    <div class="container">
        ...
        <div class="about-section">
            ...
            <form action="login_callcenter.php" method="post">
